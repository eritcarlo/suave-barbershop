...
app.get(
  ...
  (req, res) => {
    ...
    if (period === 'day') {
      ...
      dateFilter = "AND date(booking_date) >= date('now', '-12 weeks')";
    } else if (period === 'month') {
    ...
    const sql = `SELECT LOWER(service) as service_key, COUNT(*) as count FROM bookings WHERE 1=1 ${dateFilter} GROUP BY LOWER(service) ORDER BY count DESC`;
...
app.get(
  ...
  (req, res) => {
    ...
    if (period === 'day') {
      dateFilter = "AND date(bk.booking_date) >= date('now', '-30 days')";
    } else if (period === 'week') {
      dateFilter = "AND date(bk.booking_date) >= date('now', '-12 weeks')";
    } else if (period === 'month') {
      ...
      for (let i = 0; i < labels.length; i++) {
        ...
        switch (period) {
          ...
          case 'week':
            ...
            const week = Math.ceil(((currentDate - new Date(year, 0, 1)) / 86400000 + new Date(year, 0, 1).getDay() + 1) / 7);
